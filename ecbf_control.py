...
rospy.init_node('cb_drone', anonymous= True)
...
class Robot_Sim():
    ...
    def update_obstacles(self, robots, obs, noisy = False):
        ...
        goal.header.frame_id = "drone_pose"

        goal.pose.position.y = -(robot.state["x"][0])/2
        goal.pose.position.x = (robot.state["x"][1])/2
        goal.pose.position.z = 1.25
